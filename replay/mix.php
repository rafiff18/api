...
switch ($method) {
    case 'POST':
        ...
        if (
            ...
            $stmt = $db->prepare("INSERT INTO comment (users_id, event_id, comment) VALUES (?, ?, ?)");
    ...
    case 'PUT':
        ...
        if (!empty($data->comment_id)) {
            $query = "UPDATE comment SET users_id = ?, event_id = ?, comment = ? WHERE comment_id = ?";
            $stmt = $db->prepare($query);
            $stmt->bind_param("iis", $data->users_id, $data->event_id, $data->comment, $data->comment_id);
            if ($stmt->execute()) {
                http_response_code(200);
